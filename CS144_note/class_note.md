...
* 在最终查询的时候,你就可以看到`a.w.bilicdn1.com.`对应的一堆地址了,`a.w.bilicdn1.com.`是一个`CDN`服务器,至于什么是`CDN`,或许你看过`BT`那一小节会比较清楚,本质是多个在不同地区的多个服务器,以提升用户使用体验
* 值得一提的是,这里有几个问题需要解决

###### 5.4.2.1 如果任何服务器都需要首先询问根服务器,那么我怎么知道根服务器的地址和域名?

* 简单来说,根服务器的数量并不多,所以一般情况下,`DNS`解析器都会直接缓存或者永久保存所有的或者常用的几个权威服务器,所以这个现有鸡还是先有蛋的问题的结论是,`DNS`服务器手动买了一个蛋

###### 5.4.2.2 询问一个服务器看上去只会返回一个服务器的域名,我没有下一个要访问的权威服务器的地址,那该怎么访问下一个权威服务器?

* 这里我们首先要了解一下注册域名的流程
* ~~其实注册域名简单来说就是域名下放,去运营商那边申请一个域名,然后他会在顶级域名的权威服务器添加一条你的域名的`NS`,如果你的域名使用的是`com.`或者说不是`CNAME`,那么你就需要填写一下`IP`地址,也就是一个叫做`Glue`的东西,这个东西主要是防止发生无限循环访问~~

* 老实说我已经解释不清楚这个问题了,那么总结一个结论就是,如果权威服务器放在自己的域名下,那么就要使用`Glue`防止无限循环访问,当然也可以把权威服务器设置在其他服务器里,不设置在自己的域名下,那么`DNS`解析器就需要重新从头解析该权威服务器域名

* 嗯,大概是这样
* 放弃了放弃了放弃了...
* 可能等经验更加充足一点才会回来补充这个内容吧
