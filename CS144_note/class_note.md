...
* 当用户主动发出第一个包之后(当然,这个包也许是`SYN`什么的),`NAT`就会记录这个映射关系,比方说这里建立了这样的映射关系:`192.168.10.100`<-->`177.61.2.108`
* 这个外部的公网`IP`会因为这个映射关系,短暂的进入`NAT`的"白名单"
* 此时外部`IP``177.61.2.108`如果访问该设备,就只需要给`NAT`发包就行了
...
* 我们之前聊到的`NAT`都是将本地设备`IP`和外部设备`IP`做临时映射,但对于全椎`NAT`并非如此
* 他不会映射外部设备的`IP`,而是映射`NAT`自己的`IP`+`Port`
